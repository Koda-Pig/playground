window.addEventListener('load',() => {
  ...
  const restartBtn = document.querySelector('.restart-btn'),
        ...
        collisionCanvas = document.querySelector('#collision-canvas'),
  ...
  let score = 0,
      highscore = localStorage.getItem('whenPigsFlyHighScore') || 0,  // check if a high score is saved in user browser
      level = 1,
      levelCount = 0,
      gameOver = false,
      health = 10,
  ...
  class Pig {
    ...
    update(deltaTime) {
      if (this.y < 0 || this.y > canvas.height - this.height) { // to prevent pigs from going off screen on Y axis
  ...
  function animate(timestamp) {
    ...
    drawHealth();
    drawLevel();
    [ ...particles, ...pigs, ...explosions].forEach(object => object.update(deltaTime)); // array literal spread operator
