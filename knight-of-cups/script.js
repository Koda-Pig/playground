window.addEventListener('load',() => {
  ...
  let gradient = ctx.createRadialGradient(canvas.width/2, canvas.height/2, 500, canvas.width/2, canvas.height/2, 100);
  ...
  class Enemy {
    ...
    draw() {
      ctx.drawImage(this.image, this.width * this.frameX, this.height * this.frameY, this.width, this.height, this.x, this.y, this.width/2, this.height/2);
      if (this.frameX < this.maxFrame) this.frameX++;
      else this.frameX = this.minFrame;
    }
    collisionDetection() {
        // shrunken hitbox
      if (player.x + 20 < this.x + 20 + this.width/3 &&
        player.x + 20 + player.width * .6 > this.x + 20 &&
        player.y + 10 < this.y + 10 + this.height/3 &&
        player.y +10 + player.height * .8 > this.y + 10) {
          this.markedForDeletion = true;
          player.health--;
          scoreCounter++;
  ...
  function updatePlayer() {
    // checks user input
    if (keys['ArrowRight'] && keys['ArrowUp'] && player.x < canvas.width - player.width && player.y > 0) {
      player.frameY = 1;
      player.minFrame = 4;
      player.maxFrame = 14;
      player.y -= player.speed *.75;
      player.x += player.speed *.75;
    } else if (keys['ArrowRight'] && keys['ArrowDown'] && player.x < canvas.width - player.width && player.y < canvas.height - player.height) {
      player.frameY = 4;
      player.minFrame = 4;
      player.maxFrame = 15;
      player.y += player.speed *.75;
      player.x += player.speed *.75;
    } else if (keys['ArrowLeft'] && keys['ArrowDown'] && player.x > 0 && player.y < canvas.height - player.height) {
      player.frameY = 12;
      player.minFrame = 4;
      player.maxFrame = 15;
      player.y += player.speed *.75;
      player.x -= player.speed *.75;
    } else if (keys['ArrowLeft'] && keys['ArrowUp'] && player.x > 0 && player.y > 0) {
      player.frameY = 9;
      player.minFrame = 4;
      player.maxFrame = 14;
      player.y -= player.speed *.75;
  ...
  function drawHealth() {
    ctx.save();
    ctx.beginPath();
    ctx.arc(60, canvas.height - 60, 50, -1.49, -2.105 + (scoreCounter * 0.625), true);
    ctx.lineWidth = 10;
    ctx.strokeStyle = 'red';
    ctx.stroke();
